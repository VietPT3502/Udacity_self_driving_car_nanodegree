...
double Probability(Eigen::MatrixXd X, Eigen::MatrixXd Q, Eigen::MatrixXd S){
	...
	// Print the shape (dimensions) of the matrix
	Eigen::MatrixXd d = -(X-Q).transpose() * S * (X - Q);
	double p = exp(d(0, 0) * 0.5);
	return p;
}
...
void NewtonsMethod(PointT point, double theta, Cell cell, Eigen::MatrixBase<Derived>& g_previous, Eigen:: MatrixBase<Derived>& H_previous){
	...
	Eigen::MatrixXd exponential (1, 1);
	exponential(0, 0) = exp(-0.5  *(q.transpose()*  Si * q)(0,0) );
    ...
    g(0, 0 ) = (q.transpose() * Si * dx * exponential)(0,0);
	g(1, 0 ) = (q.transpose() * Si * dy * exponential)(0,0);
	g(2, 0 ) = (q.transpose() * Si * dtheta * exponential)(0,0);
    // TODO: calculate the 2 x 1 second order partial derivative matrix
	Eigen::MatrixXd ddtheta(2, 1);
	ddtheta(0, 0) = -point.x * cos(theta) + point.y * sin(theta);
	ddtheta(1, 0) = -point.x * sin(theta) - point.y * cos(theta);
	...
	H(0, 0 ) = (- exponential * ((-q.transpose() *Si * dx) * (-q.transpose() * Si * dx)
								+ (-dx.transpose() * Si * dx)))(0, 0);

	H(0, 1 ) = (- exponential * ((-q.transpose() * Si * dx) * (-q.transpose() * Si * dy)
								+ (-dy.transpose() * Si * dx)))(0, 0);

	H(0, 2 ) = (- exponential * ((-q.transpose() * Si * dx) * (-q.transpose() * Si * dtheta)
								+ (-dtheta.transpose() * Si * dx)))(0, 0);
	// second row
	H(1, 0 ) = (- exponential * ((-q.transpose() * Si * dy) * (-q.transpose() * Si * dx)
								+ (-dx.transpose() * Si * dy)))(0, 0);

	H(1, 1 ) = (- exponential * ((-q.transpose() * Si * dy) * (-q.transpose() * Si * dy)
								+ (-dy.transpose() * Si * dy)))(0, 0);

	H(1, 2 ) = (- exponential * ((-q.transpose() * Si * dy) * (-q.transpose() * Si * dtheta)
								+ (-dtheta.transpose() * Si * dy)))(0, 0);

	// third row
	H(2, 0 ) = (- exponential * ((-q.transpose() * Si * dtheta) * (-q.transpose() * Si * dx)
								+ (-dx.transpose() * Si * dtheta)))(0, 0);

	H(2, 1 ) = (- exponential * ((-q.transpose() * Si * dtheta) * (-q.transpose() * Si * dy)
								+ (-dy.transpose() * Si * dtheta)))(0, 0);

	H(2, 2 ) = (- exponential * ((-q.transpose() * Si * dtheta) * (-q.transpose() * Si * dtheta)
								+ (-q.transpose() * Si * ddtheta)
								+ (-dtheta.transpose() * Si * dtheta)))(0, 0);
	H_previous += H;
	g_previous += g;
			...
			if(matching){
				...
				for(PointT point:source->points){
					...
					if(cell.cloud->points.size() > 2){
						...
						PointT pointTran(x * cos(theta) - y * sin(theta) + x, x * sin(theta) + y * cos(theta) + y, point.z);
