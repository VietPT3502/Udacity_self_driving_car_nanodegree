...
vector<int> NN(PointCloudT::Ptr target, PointCloudT::Ptr source, Eigen::Matrix4d initTransform, double dist){
	...
	for(PointT cloud : transformSource->points)
	{
		std::vector<int> pointIdxRadiusSearch;
		std::vector<float> pointRadiusSquaredDistance;
		if (kdtree.radiusSearch(cloud, dist, pointIdxRadiusSearch, pointRadiusSquaredDistance) > 0)
...
Eigen::Matrix4d ICP(vector<int> associations, PointCloudT::Ptr target, PointCloudT::Ptr source, Pose startingPose, int iterations, pcl::visualization::PCLVisualizer::Ptr& viewer){
    ...
    pcl::transformPointCloud (*source,* transformSource, initTransform);
  	...
  	//	   [ mean p1 y]	    [ mean p2 y]
	vector<Pair> pairs = PairPoints(associations, target, source, false, viewer);
	...
	transformation_matrix(0, 3) = R(0, 0);
	transformation_matrix(1, 3) = R(0, 0);
