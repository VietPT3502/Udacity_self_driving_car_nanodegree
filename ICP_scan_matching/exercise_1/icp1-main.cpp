...
Eigen::Matrix4f ICP(PointCloudT::Ptr target, PointCloudT::Ptr source, Pose startingPose, int iterations){


    Eigen::Matrix4f transformation_matrix = Eigen::Matrix4f::Identity ();
    // align source with starting pose
      Eigen::Matrix4f initTransform = transform2D(startingPose.theta, startingPose.position.x, startingPose.position.y);
...
int main(){
    ...
    if(true){ // Change to true
        srand(time(0));
        for(int i = 0; i < 10; i++){
            float mag = 0.5 * ((float) rand() / (RAND_MAX)) + 0.5;
            float angle = pi/8 * ((float) rand() / (RAND_MAX)) + pi/8;
    ...
    for( Vect2 move : movement ){
        ...
        // PointCloudT::Ptr singlePointCloud(new PointCloudT);
        // PointT singlePoint;
        // singlePoint.x = 1.0f;
        // singlePoint.y = 2.0f;
        // singlePoint.z = 3.0f;
        // singlePointCloud->push_back(singlePoint);
        // singlePointCloud->push_back(singlePoint);
        // singlePointCloud->push_back(singlePoint);
        // singlePointCloud->push_back(singlePoint);
        // // Print the coordinates of the single point
        // std::cout << "Single Point: X: " << singlePoint.x
        //         << ", Y: " << singlePoint.y
        //         << ", Z: " << singlePoint.z << std::endl;
        // std::cout << *singlePointCloud << endl;
        // std::cout << scan << endl;
        // PointCloudT::Ptr transformSource (new PointCloudT);
        // Eigen::Matrix4f initTransform = transform2D(0.5f, 2, 3);
        // Eigen::Matrix4f transformation_matrix = Eigen::Matrix4f::Identity ();
        // // align source with starting pose
        // pcl::transformPointCloud (*singlePointCloud,*transformSource, initTransform);
        // std::cout << "Done" << transformSource << endl;

        // perform localization
        Eigen::Matrix4f transform = ICP(map, scan, location, 50); //TODO: make the iteration count greater than zero
