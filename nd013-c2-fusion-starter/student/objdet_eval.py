...
def measure_detection_performance(detections, labels, labels_valid, min_iou=0.5):
    ...
    for label, valid in zip(labels, labels_valid):
        ...
        if valid: # exclude all labels from statistics which are not considered valid
            ...
            candidate_box = tools.compute_box_corners(candidate[1], candidate[2], candidate[5], candidate[6], candidate[7])
            ## step 2 : loop over all detected objects
            for detection in detections:
                ## step 3 : extract the four corners of the current detection
                box = tools.compute_box_corners(detection[1], detection[2], detection[5], detection[6], detection[7])
                ## step 4 : computer the center distance between label and detection bounding-box in x, y, and z
                distance_x = np.sqrt(abs(np.power(candidate[1], 2) - np.power(detection[1], 2)))
                distance_y = np.sqrt(abs(np.power(candidate[2], 2) - np.power(detection[2], 2)))
                distance_z = np.sqrt(abs(np.power(candidate[3], 2) - np.power(detection[3], 2)))
                ## step 5 : compute the intersection over union (IOU) between label and detection bounding-box
                candidate_polygon = Polygon(candidate_box)
                detection_polygon = Polygon(box)
                intersect =  candidate_polygon.intersection(detection_polygon).area
                union = candidate_polygon.area + detection_polygon.area - intersect
                iou = intersect / union
...
def compute_performance_stats(det_performance_all):
    ...
    total_positives = np.array(pos_negs)[:, 0 ]
    true_positives = np.array(pos_negs)[:, 1]
    false_negatives = np.array(pos_negs)[:, 2]
    false_positives = np.array(pos_negs)[:, 3]
