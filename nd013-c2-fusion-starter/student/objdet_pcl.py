...
def show_range_image(frame, lidar_name):
    ...
    range = (range * 255 / (np.amax(range) - np.amin(range))).astype(int)
 
    # step 5 : map the intensity channel onto an 8-bit scale and normalize with the difference between the 1- and 99-percentile to mitigate the influence of outliers
    intensity = (np.amax(intensity) / 2 *  intensity * 255 / (np.amax(intensity) - np.amin(intensity))).astype(int)
    deg45 = int(range.shape[1] / 8)
    ri_center = int(range.shape[1]/2)
    range = range[:,ri_center-deg45:ri_center+deg45]
    intensity = intensity[:,ri_center-deg45:ri_center+deg45]
...
def bev_from_pcl(lidar_pcl, configs, vis=False):
    ...
    indexes = np.lexsort((lidar_pcl_cpy[:, 0], lidar_pcl_cpy[:, 1], -lidar_pcl_cpy[:, 2]))
