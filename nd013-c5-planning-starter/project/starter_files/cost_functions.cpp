...
double collision_circles_cost_spiral(const std::vector<PathPoint>& spiral,
  ...
  for (auto wp : spiral) {
    ...
    for (size_t c = 0; c < n_circles && !collision; ++c) {
      ...
      auto circle_center_x = CIRCLE_OFFSETS[c] * std::cos(cur_yaw);  // <- Update 
      auto circle_center_y = CIRCLE_OFFSETS[c] * std::sin(cur_yaw);  // <- Update 
      ...
      for (auto obst : obstacles) {
        ...
        for (size_t c2 = 0; c2 < n_circles && !collision; ++c2) {
          ...
          // obstacle/actor
          double dist = std::sqrt(std::pow(circle_center_x - obst.location.x, 2) +
                                  std::pow(circle_center_y - obst.location.y, 2));  // <- Update
...
double close_to_main_goal_cost_spiral(const std::vector<PathPoint>& spiral,
  ...
  auto delta_x = std::abs(spiral[n-1].x - main_goal.location.x);  // <- Update
  auto delta_y = std::abs(spiral[n - 1].y - main_goal.location.y);  // <- Update
  auto delta_z = std::abs(spiral[n - 1].z - main_goal.location.z);  // <- Update
